...
def predict_all(model_config_path, model_weights_path, video_dir, out_dir):
    pose_predictor = init_pose_predictor(
        model_config_path, model_weights_path, cuda=True)
    i = 0
    for filename in os.listdir(video_dir):
        if filename[-10:] == "_50fps.mp4":
            i += 1
            print(f"Processing file #{i}: ", filename)
            out_name = "data_2d_custom_" + filename[:-4]
            img_generator = read_video(video_dir + "/" + filename)
            predict_pose(pose_predictor, img_generator, out_dir + "/" + out_name)
        else:
            continue
